...
//UART rate settings
// This is strange, Make sure this check works...
#if(DISPLAY_VER == HW_DISP_CU3)
	uint8_t bscale = 0;
	uint8_t bsel = 103; //b19200 103
#else
	uint8_t bscale = 0;
	uint8_t bsel = 207; //b9600n 
#endif
...
void uart0_init(){
    ...
    USARTE0.BAUDCTRLA = (bsel & 0xff) << USART_BSEL_gp;
    USARTE0.BAUDCTRLB = ((bscale) << USART_BSCALE_gp) | ((bsel >> 8) << USART_BSEL_gp);
